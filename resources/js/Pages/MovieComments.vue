...
function addComment() {
    ...
    comment.post(route('add.comment', { 'movie': props.movie.id }),
        {
            onSuccess: (res) => {
                // router.visit('');
                console.log(res)
    ...
    <AppLayout :title="movie.title + 's Comments'">
        ...
        <div class="py-10 max-w-7xl mx-auto sm:px-6 lg:px-8 bg-white flex flex-col gap-5 ">
            ...
            <form @submit.prevent="addComment" method="post" class="flex flex-row p-2 px-40 w-full">
                <input type="text" :placeholder="'Lets comment about ' + movie.title" v-model="comment.content"
                    required class="rounded-s-md bg-transparent border-r-0 border-2 focus:border-yellow-500 w-full" />
                <input id="score" type="number" v-model="comment.score" min="0" max="5" />
